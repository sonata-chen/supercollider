...
static bool findInBlock(QTextDocument* doc, const QTextBlock& block, const QRegularExpression& expr, int offset,
    ...
    QRegularExpressionMatch inner_match;
    if (!match) {
        match = &inner_match;
    }
...
static QString resolvedReplacement(const QString& replacement, const QRegularExpressionMatch& match) {
    // qDebug("START");
    ...
    while (i < str.size()) {
        ...
        if (!backref_match.captured(1).isEmpty()) {
            // qDebug("%lli (%s): escape", i, CSTR(backref_match.captured(1)));
            str.replace(i, len, "\\");
            i += 1;
        } else if (!backref_match.captured(2).isEmpty()) {
...
bool GenericCodeEditor::replace(const QRegularExpression& expr, const QString& replacement,
    ...
    if (cursor.hasSelection()) {
        ...
        QRegularExpressionMatch match = expr.match(selectedText);
        if (match.hasMatch()) {
            QString rstr = replacement;
            rstr = resolvedReplacement(rstr, match);
